...
export function MenuContainer({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="fixed left-4 top-1/2 -translate-y-1/2 z-50 w-[64px]" data-expanded={isExpanded}>
      <div className="relative">
        <div 
          className="relative w-16 h-16 bg-gray-100/10 backdrop-blur-sm cursor-pointer rounded-full group will-change-transform z-50"
        ...
        {childrenArray.slice(1).map((child, index) => (
          <div 
            key={index} 
            className="absolute top-0 left-0 w-16 h-16 bg-gray-100/10 backdrop-blur-sm will-change-transform rounded-full"
            style={{
